...
const AUTH_FEATURES = [
  {
    ...
    description:
      "Verify students quickly with AI-assisted face matching and liveness-aware attendance flows.",
  },
  {
    title: "Department-ready controls",
    description:
      "Manage subjects, teachers, lectures, timetables, and notifications in one clean academic workspace.",
  },
  {
    title: "Real-time attendance insight",
    description:
      "Track attendance updates, alerts, and reports instantly across student, teacher, and admin dashboards.",
  },
];
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.requiresAdminScope)
        sessionStorage.removeItem("adminScopeSelected");
      toast.success(`Welcome back, ${res.data.user.name}!`);
      navigate(
        ["admin", "teacher"].includes(res.data.user.role)
          ? "/admin"
          : "/student",
      );
    } catch (err) {
  ...
  const handleBiometricLogin = async () => {
    ...
    try {
      const optionsResponse = await authAPI.beginBiometricLogin({
        credentialId,
      });
      ...
      toast.success(
        verifyResponse.data.message ||
          `Welcome back, ${verifyResponse.data.user.name}!`,
      );
      navigate("/student");
    } catch (err) {
      const message =
  ...
  return (
    <div className="auth-login-page min-h-dvh flex items-center justify-center relative overflow-hidden px-3 py-6">
      ...
      >
        <motion.div
          className="glass-card hidden min-h-[520px] overflow-hidden p-8 lg:flex lg:flex-col lg:justify-between"
          initial="initial"
          animate="animate"
        >
          <div>
            ...
            <motion.h1
              className="auth-login-title font-display text-5xl font-bold leading-tight"
              variants={fieldMotion}
            ...
            <motion.p
              className="auth-login-subtitle mt-3 text-lg"
              variants={fieldMotion}
              transition={{ duration: 0.32, delay: 0.1 }}
            >
              Attendance, classrooms, and conversations in one place
            </motion.p>
          </div>
          ...
          <div className="relative rounded-2xl border border-white/10 bg-white/[0.04] p-5">
            ...
            >
              ...
              <h2 className="mt-3 text-2xl font-semibold text-white">
                {AUTH_FEATURES[activeFeature].title}
              </h2>
              <p className="mt-3 max-w-md text-sm leading-6 text-slate-300">
                {AUTH_FEATURES[activeFeature].description}
              </p>
            </motion.div>
        ...
        <motion.div
          className="auth-login-card auth-login-compact glass-card mx-auto w-full max-w-sm"
          whileHover={{ y: -2 }}
        ...
        >
          <div className="mb-5 text-center lg:hidden">
            ...
            <h1 className="auth-login-title font-display text-2xl font-bold">
              StudySphere
            </h1>
            <p className="auth-login-subtitle mt-0.5 text-xs">
              Campus learning workspace
            </p>
          </div>
          <motion.h2
            className="auth-login-heading mb-4 text-lg font-semibold"
            {...fieldMotion}
          ...
          <motion.div
            className="auth-login-divider mt-4 pt-4"
            {...fieldMotion}
            ...
            >
              ...
              >
                {biometricLoading ? (
                  ...
                  <>
                    <Fingerprint className="h-4 w-4" /> Login with Biometric
                  </>
                )}
              </button>
            </motion.div>
          ...
          <motion.div
            className="auth-login-divider mt-4 pt-4 text-center"
            {...fieldMotion}
          ...
          <motion.div
            className="auth-login-secure mt-3 flex items-center gap-2 rounded-lg p-2.5"
            {...fieldMotion}
            transition={{ delay: 0.3 }}
          >
            <ShieldCheck className="h-3.5 w-3.5 flex-shrink-0 text-emerald-400" />
            <p className="text-[11px] sm:text-xs">
              Secured with face recognition & end-to-end encryption
            </p>
          </motion.div>
        </motion.div>
      </motion.div>
