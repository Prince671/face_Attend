...
export default function StudentNotifications() {
  ...
  // Real-time: refresh list when a new lecture or attendance event arrives
  useEffect(() => {
    if (!socket) return;
    const refresh = () => fetchNotifs();
    socket.on('new_lecture', refresh);
    socket.on('attendance_opened', refresh);
    socket.on('attendance_closed', refresh);
    return () => {
      socket.off('new_lecture', refresh);
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <AdminBreadcrumb items={[{ label: 'Student Portal' }, { label: 'Notifications' }]} />
